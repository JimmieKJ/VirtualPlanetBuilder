...
public:
	...
	void _getMinMaxXYInGeom(ArrayType & array)
	{
		...
		//m_vertexAllArray = new osg::Vec4Array(vCount);
		...
		for (int vi = 0; vi < vCount; vi++)
		{
			...
			m_vertexAllArray.push_back(osg::Vec4(tempx, tempy,0.0,0.0));
			if (tempx > maxVX)
			{
				maxVX = tempx;
			}
	...
	std::vector<osg::Vec4> m_vertexAllArray;
};
    ...
    public:
		...
		int _computeSimilarHeight(DataType dataTypeIn, int dataWidth,int dataHeight, void* demDataTemp)
		{
			...
			switch (dataTypeIn) {
				case kUByte: {
					...
					float rbvaldat = abs((ltvaldat + lcvaldat + lbvaldat + ctvaldat + cbvaldat + rtvaldat + rcvaldat) / 7);
					minVal = rbvaldat;
					index = 1;
					
					i = dataWidth - 2;
					j = 1;
					ltindex = ((j - 1) >= 0 && (i - 1) >= 0) ? ((j - 1) * dataWidth + (i - 1)) : -1;
					lcindex = ((i - 1) >= 0) ? (j * dataWidth + (i - 1)) : -1;
					lbindex = (j + 1 < dataHeight && (i - 1) >= 0) ? ((j + 1) * dataWidth + (i - 1)) : -1;
					ctindex = ((j - 1) >= 0) ? ((j - 1) * dataWidth + i) : -1;
					ccindex = j * dataWidth + i;
					cbindex = ((j + 1) < dataHeight) ? ((j + 1) * dataWidth + i) : -1;
					rtindex = ((j - 1) >= 0 && (i + 1) < dataWidth) ? ((j - 1) * dataWidth + i + 1) : -1;
					rcindex = ((i + 1) < dataWidth) ? (j * dataWidth + i + 1) : -1;
					rbindex = ((j + 1) < dataHeight && (i + 1) < dataWidth) ? ((j + 1) * dataWidth + i + 1) : -1;
					val = demData[ccindex];
					ltvaldat = (ltindex >= 0 && ltindex < dataLength) ? (val - demData[ltindex]) : 0;
					lcvaldat = (lcindex >= 0 && lcindex < dataLength) ? (val - demData[lcindex]) : 0;
					ctvaldat = (ctindex >= 0 && ctindex < dataLength) ? (val - demData[ctindex]) : 0;
					cbvaldat = (cbindex >= 0 && cbindex < dataLength) ? (val - demData[cbindex]) : 0;
					rtvaldat = (rtindex >= 0 && rtindex < dataLength) ? (val - demData[rtindex]) : 0;
					rcvaldat = (rcindex >= 0 && rcindex < dataLength) ? (val - demData[rcindex]) : 0;
					rbvaldat = (rbindex >= 0 && rbindex < dataLength) ? (val - demData[rbindex]) : 0;
					lbvaldat = abs((ltvaldat + lcvaldat + rbvaldat + ctvaldat + cbvaldat + rtvaldat + rcvaldat) / 7);
					if (minVal > lbvaldat) {
						minVal = lbvaldat;
						index = 2;
					}
					...
					ltvaldat = (ltindex >= 0 && ltindex < dataLength) ? (val - demData[ltindex]) : 0;
					lcvaldat = (lcindex >= 0 && lcindex < dataLength) ? (val - demData[lcindex]) : 0;
					lbvaldat = (lbindex >= 0 && lbindex < dataLength) ? (val - demData[lbindex]) : 0;
					ctvaldat = (ctindex >= 0 && ctindex < dataLength) ? (val - demData[ctindex]) : 0;
					cbvaldat = (cbindex >= 0 && cbindex < dataLength) ? (val - demData[cbindex]) : 0;
					rcvaldat = (rcindex >= 0 && rcindex < dataLength) ? (val - demData[rcindex]) : 0;
					rbvaldat = (rbindex >= 0 && rbindex < dataLength) ? (val - demData[rbindex]) : 0;
					rtvaldat = abs((ltvaldat + lcvaldat + rbvaldat + ctvaldat + cbvaldat + lbvaldat + rcvaldat) / 7);
					if (minVal > rtvaldat) {
						minVal = rtvaldat;
						index = 3;
					}
					...
					rcvaldat = (rcindex >= 0 && rcindex < dataLength) ? (val - demData[rcindex]) : 0;
					rbvaldat = (rbindex >= 0 && rbindex < dataLength) ? (val - demData[rbindex]) : 0;
					ltvaldat = abs((rtvaldat + lcvaldat + rbvaldat + ctvaldat + cbvaldat + lbvaldat + rcvaldat) / 7);
					if (minVal > ltvaldat) {
						minVal = ltvaldat;
						index = 4;
					}
				...
				case kFloat: {
					...
					int i = 1;
					int j = 1;
					int ltindex = ((i - 1) >= 0 && (j - 1) >= 0) ? ((j - 1) * dataWidth + (i - 1)) : -1;
					int lcindex = ((i - 1) >= 0) ? (j * dataWidth + (i - 1)) : -1;
					int lbindex = (j + 1 < dataHeight && (i - 1) >= 0) ? ((j + 1) * dataWidth + (i - 1)) : -1;
					int ctindex = ((j - 1) >= 0) ? ((j - 1) * dataWidth + i) : -1;
					int ccindex = j * dataWidth + i;
					...
					rcvaldat = (rcindex >= 0 && rcindex < dataLength) ? (val - demData[rcindex]) : 0;
					rbvaldat = (rbindex >= 0 && rbindex < dataLength) ? (val - demData[rbindex]) : 0;
					lbvaldat = (ltvaldat + lcvaldat + rbvaldat + ctvaldat + cbvaldat + rtvaldat + rcvaldat) / 7;
					lbvaldat = abs(lbvaldat);
					if (minVal > lbvaldat) {
						minVal = lbvaldat;
						index = 2;
					}
					...
					rcvaldat = (rcindex >= 0 && rcindex < dataLength) ? (val - demData[rcindex]) : 0;
					rbvaldat = (rbindex >= 0 && rbindex < dataLength) ? (val - demData[rbindex]) : 0;
					rtvaldat = (ltvaldat + lcvaldat + rbvaldat + ctvaldat + cbvaldat + lbvaldat + rcvaldat) / 7;
					rtvaldat = abs(rtvaldat);
					if (minVal > rtvaldat) {
						minVal = rtvaldat;
						index = 3;
					}
					...
					rcvaldat = (rcindex >= 0 && rcindex < dataLength) ? (val - demData[rcindex]) : 0;
					rbvaldat = (rbindex >= 0 && rbindex < dataLength) ? (val - demData[rbindex]) : 0;
					ltvaldat = (rtvaldat + lcvaldat + rbvaldat + ctvaldat + cbvaldat + lbvaldat + rcvaldat) / 7;
					ltvaldat = abs(ltvaldat);
					if (minVal > ltvaldat) {
						minVal = ltvaldat;
						index = 4;
					}
		...
		void _computeDEMData(void* pDataIn, void* demDataTemp,DataType dataTypeIn, int dataWidth, int dataHeight,double& maxBudingHeight)
		{
			...
			case kFloat: {
				...
				for (int j = 1; j < dataHeight / 2; j++)
				{
					for (int i = 1; i < dataWidth / 2; i++)
					{
						int ltindex = ((j - 1)>=0 && (i - 1)>=0)?((j - 1) * dataWidth + (i - 1)):-1;
						int lcindex = ((i-1)>=0) ? (j * dataWidth + (i - 1)) : -1;
						int lbindex = (j+1< dataHeight && (i - 1) >= 0)?((j + 1) * dataWidth + (i - 1)):-1;
						...
						if (abs(ltvaldat) > _maxLandVal)
							ltvaldat = 0.0;
						if (abs(lcvaldat) > _maxLandVal)
							lcvaldat = 0.0;
						if (abs(lbvaldat) > _maxLandVal)
							lbvaldat = 0.0;
						if (abs(ctvaldat) > _maxLandVal)
							ctvaldat = 0.0;
						if (abs(cbvaldat) > _maxLandVal)
							cbvaldat = 0.0;
						if (abs(rtvaldat) > _maxLandVal)
							rtvaldat = 0.0;
						if (abs(rcvaldat) > _maxLandVal)
							rcvaldat = 0.0;
						/*if (abs(rbvaldat) > _maxLandVal)
							rbvaldat = 0.0;*/
					...
					for (int i = dataWidth - 2; i >= dataWidth / 2; i--)
					{
						int ltindex = ((j - 1) >= 0 && (i - 1) >= 0) ? ((j - 1) * dataWidth + (i - 1)) : -1;
						int lcindex = ((i - 1) >= 0) ? (j * dataWidth + (i - 1)) : -1;
						int lbindex = (j + 1 < dataHeight && (i - 1) >= 0) ? ((j + 1) * dataWidth + (i - 1)) : -1;
				...
				for (int j = dataHeight - 2; j >= dataHeight / 2; j--)
				{
					for (int i = 1; i < dataWidth / 2; i++)
					{
						int ltindex = ((j - 1) >= 0 && (i - 1) >= 0) ? ((j - 1) * dataWidth + (i - 1)) : -1;
		...
		void* _getDemOriginValFromDsmData(void* pDataIn, DataType dataTypeIn, int dataWidth, int dataHeight)
		{
			...
			switch (dataTypeIn) {
				case kUByte: {
					...
					for (int j = 0; j < dataHeight; j++) {
						for (int i = 0; i < dataWidth; i++) {
							//int dindex = j * dataWidth + i;
							int index = j * dataWidth + i;
							if (bReverseY)
							{
								index = (dataHeight-1-j)*dataWidth + i;
							}
							if (i < 2) {
        ...
        virtual void apply(osg::Geode& node)
        {
            ...
            if (!bTraversed && overlap(bb.xMin(), bb.yMin(), bb.xMax(), bb.yMax()))
            {
				...
				for (unsigned int i = 0; i < numDrawables; ++i)
				{
					...
					std::vector<osg::Vec4> allVertexArray = daif.m_vertexAllArray;
		...
		double distancePointToLine(const osg::Vec2 & pt, const osg::Vec2 & lineStartPt, const osg::Vec2 & lineEndPt)
		{
			double distance = 0.0;
			double area = (lineStartPt.y() - lineEndPt.y())*pt.x() + (lineEndPt.x() - lineStartPt.x())*pt.y() + (lineStartPt.x()*lineEndPt.y() - lineEndPt.x()*lineStartPt.y());
			double linelen = sqrt(pow((lineEndPt.x() - lineStartPt.x()), 2) + pow((lineEndPt.y() - lineStartPt.y()), 2));
			distance = area / linelen;
			return distance;
		}
